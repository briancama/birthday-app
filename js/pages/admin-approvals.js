...
export class AdminApprovalsPage extends BasePage {
  ...
  populateAssignmentModal(title) {
    ...
    if (infoDiv) {
      ...
      const actualAssignments = this.currentChallenge.actual_assignments && this.currentChallenge.actual_assignments.length > 0
        ? this.currentChallenge.actual_assignments.map(a => a.display_name || a.username).join(', ')
        : null;
  ...
  async handleAssignmentFormSubmit(e) {
    ...
    const hasChanges = currentSet.size !== selectedSet.size ||
      [...currentSet].some(x => !selectedSet.has(x));

    if (!hasChanges) {
      this.showAssignmentError('No changes detected. Please check or uncheck users to modify assignments.');
      return;
    }
    ...
    try {
      ...
      const successMessage = this.generateSuccessMessage(isApproval, operations, selectedUserIds.length);
  ...
  generateSuccessMessage(isApproval, operations, finalAssignmentCount) {
    ...
    if (operations && operations.length > 0) {
      ...
      if (counts.reactivated > 0) {
        messages.push(`reactivated ${counts.reactivated} user${counts.reactivated !== 1 ? 's' : ''}`);
      }
      if (counts.deactivated > 0) {
        messages.push(`unassigned ${counts.deactivated} user${counts.deactivated !== 1 ? 's' : ''}`);
      }
    }

    const actionMsg = messages.length > 0
      ? messages.join(' and ')
