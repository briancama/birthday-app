...
class AudioManager {
  constructor() {
    // sounds: { [name]: { audio: HTMLAudioElement, lazy: boolean } }
    this.sounds = {};
    this.enabled = true;
  ...
  preload(name, src, lazy = false) {
    if (!this.sounds[name]) {
      ...
      this.sounds[name] = { audio, lazy: !!lazy };
    }
  }
  ...
  setVolume(volume) {
    Object.values(this.sounds).forEach((entry) => {
      if (entry && entry.audio) entry.audio.volume = Math.max(0, Math.min(1, volume));
    });
  }
  ...
  initialize() {
    ...
    // Only load sounds that were requested eagerly (lazy === false)
    Object.values(this.sounds).forEach((entry) => {
      try {
        if (entry && entry.audio && !entry.lazy) entry.audio.load();
      } catch (e) {
